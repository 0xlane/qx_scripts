...
获取Cookie说明：
打开京东App后, 点击“我的”，如通知成功获取Cookie则可以使用该脚本.

*********************************
Surge(iOS 5.9.0+/macOS 5.5.0+)模块：
https://raw.githubusercontent.com/0xlane/qx_scripts/refs/heads/main/DailyJobs/Surge/Module/JDBeanDailyJob.sgmodule

*********************************
QuantumultX 任务仓库(Gallery)订阅：
https://raw.githubusercontent.com/0xlane/qx_scripts/refs/heads/main/DailyJobs/DailyJobs.json

工具&分析->HTTP请求->右上角添加任务仓库->选择京东京豆签到签到脚本添加定时任务和附加组件

*********************************
Loon 脚本订阅(非插件)：
https://raw.githubusercontent.com/0xlane/qx_scripts/refs/heads/main/DailyJobs/Loon/JDBeanDailyJob.plugin

添加后请按需启用脚本

*********************************/

const SCRIPT_NAME = "京东京豆签到";
...
var url_fetch_sign = {
    ...
    headers: {
        ...
        "User-Agent": "JD4iPhone/13.6.5 CFNetwork/1568.200.51 Darwin/24.1.0"
    }
};
...
function signJDBean() {
    ...
    $core.get(url_fetch_sign, function(error, response, body){
        if (error) {
            $core.notify(SCRIPT_NAME, "签到失败", JSON.stringify(error));
            return $core.done();
        }
        ...
        if (body) {
            ...
            if (data.code != 0) {
                $core.notify(SCRIPT_NAME, "签到失败", body);
                return $core.done();
            } else {
                if (data && data.data && data.data.status) {
                    if (data.data.status == 1) {
                        if (data.data.dailyAward && data.data.dailyAward.beanAward && data.data.dailyAward.beanAward.beanCount) {
                            $core.notify(SCRIPT_NAME, "签到成功", `获得${data.data.dailyAward.beanAward.beanCount}个京豆`);
                            return $core.done();
                        } else if (data.data.continuityAward && data.data.continuityAward.beanAward && data.data.continuityAward.beanAward.beanCount) {
                            $core.notify(SCRIPT_NAME, "签到成功", `获得${data.data.continuityAward.beanAward.beanCount}个京豆`);
                            return $core.done();
                        } else {
                            try {
                                var regex = /beanCount\":\"(\d+)\".+今天/;
                                var quantity = regex.exec(body)[1];
                                $core.notify(SCRIPT_NAME, "签到成功", `获得${quantity}个京豆`);
                                return $core.done();
                            } catch (error) {
                                $core.notify(SCRIPT_NAME, "签到失败", body);
                                return $core.done();
                        ...
                        if (body.match(/(已签到|新人签到)/)) {
                            $core.notify(SCRIPT_NAME, "已签到，无需重复签到");
                            return $core.done();
                        } else {
                            $core.notify(SCRIPT_NAME, "签到失败", body);
                            return $core.done();
            ...
            $core.notify(SCRIPT_NAME, "签到失败", `状态码：${response.statusCode}`);
            return $core.done();
        }
    });
}
...
function GetCookie() {
    if ($request.url.startsWith("https://api.m.jd.com/")) {
        var curCookie = $request.headers["Cookie"];
        if (curCookie && curCookie.includes("pt_key") && curCookie != cookieJD) {
